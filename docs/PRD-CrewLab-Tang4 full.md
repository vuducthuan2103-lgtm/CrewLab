...
**Định nghĩa:** Direct Assign là giao thẳng cho một agent cụ thể và bỏ qua A01. Chat/giao việc cho A01 là luồng Orchestrator chuẩn, không phải Direct Assign. T20 defer khỏi Phase 1.

```
┌──────────────────────────────────────────────────────┐
│  ⚡ Giao việc nhanh                                  │
│  Chọn agent cụ thể: [D01/D02/...]                    │
...
- Form: chọn agent → brief tự do → chọn content item tham chiếu (tùy chọn).
- **Business rule bắt buộc:** dù giao thẳng D02, **E01 luôn chấm điểm visual trước khi trả kết quả** — không có đường tắt bỏ qua Evaluator.
- **Không tạo cycle mới**, kết quả trả về ngay trong session.
- Phase 1 thay form brief bằng chat nhiều lượt với A01; A01 hỏi làm rõ và tự điều phối agent con.
...
Không thuộc Phase 1. Internal debug view hiện tại chỉ đọc, không có reopen, retry hoặc manual state override. Nếu đưa các action này trở lại ở phase sau phải có spec/decision mới và audit log đầy đủ.
