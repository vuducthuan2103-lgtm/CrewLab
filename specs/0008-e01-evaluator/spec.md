...
| 4 | 🟡 Audit | `attempt_number` bị reuse sau reopen | Ghi chú rõ: sort bằng `created_at` |
...
# Ghi chú: attempt_number bị reuse sau Agency Admin reopen
...
**Ghi chú `attempt_number`:** Không có UNIQUE constraint trên `(content_item_id, attempt_number)` vì sau khi Agency Admin reopen từ `rejected`, `eval_retry_count` reset → attempt_number bắt đầu lại từ 1. **Sort bằng `created_at` khi đọc lịch sử, không dùng attempt_number làm unique key.**
...
| Manual restart endpoint (`POST /api/items/{id}/restart`) — reopen từ `rejected` | 0009 |
| `rejected` → reset `eval_retry_count=0` + re-dispatch D01 | 0009 (cần audit log + `reopened_count`) |
