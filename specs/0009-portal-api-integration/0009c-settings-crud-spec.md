...
| `/api/v1/portal/settings/agent-config` | PATCH | Model/tier/budget per agent (PRD B5) — chỉ trong phạm vi 6 agent MVP; Portal không quản lý provider |
...
2. Validate `model` thuộc catalog hỗ trợ **và** provider của model đã được Agency Admin enable cho client. Provider được xác định từ catalog model ở backend, không nhận từ Portal request.
3. Update model/tier/budget của `client_config.llm_config.per_agent[agent_code]`; nếu model thuộc provider đang được enable khác, provider per-agent được backend cập nhật theo model đã chọn.
