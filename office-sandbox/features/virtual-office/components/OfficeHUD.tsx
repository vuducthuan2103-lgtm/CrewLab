  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-40 flex flex-col justify-between p-4 select-none">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        {/* Brand & Office Title */}
        <div className="pointer-events-auto flex items-center gap-3 px-3.5 py-2 rounded-2xl bg-[#0d0d12]/95 backdrop-blur-xl border border-zinc-800/90 shadow-2xl">
          <div className="w-8 h-8 rounded-xl bg-[#D4FF00] text-[#09090B] flex items-center justify-center font-black text-sm shadow-md shadow-[#D4FF00]/20">
            CL
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-black text-white tracking-tight">CrewLab F&B</span>
              <span className="px-1.5 py-0.2 rounded text-[9px] font-black bg-[#D4FF00]/15 text-[#D4FF00] border border-[#D4FF00]/30">
                SANDBOX
              </span>
            </div>
            <div className="text-[10px] text-zinc-400">
              Quán: <span className="text-zinc-200 font-semibold">Bardinh Coffee & Tea</span>
            </div>
          </div>
        </div>

        {/* ── TOP PRIMARY TABS SWITCHER ── */}
        ...
        {/* ── RIGHT STREAMLINED ACTION & LIVE BADGES ── */}
        <div className="pointer-events-auto flex items-center gap-2">
          {/* Live Active Status */}
          <div className="hidden xl:flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-[#0d0d12]/95 backdrop-blur-xl border border-zinc-800 text-[11px] font-medium text-zinc-300 shadow-xl">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span>{summary.workingCount} Agent đang làm</span>
          </div>

          {/* 🔔 SMART ATTENTION QUEUE BUTTON */}
          <button
            onClick={() => setAttentionQueueOpen(true)}
            className={`px-3.5 py-2 rounded-2xl backdrop-blur-xl border text-xs font-bold flex items-center gap-2 shadow-2xl transition-all ${
              summary.waitingForCeoCount > 0
                ? 'bg-amber-500/20 border-amber-400/60 text-amber-300 hover:bg-amber-500/30 animate-pulse shadow-amber-500/20 cursor-pointer'
                : 'bg-[#0d0d12]/95 border-zinc-800 text-zinc-400 hover:text-white'
            }`}
            title="Hàng đợi công việc cần CEO phê duyệt"
          >
            <AlertCircle className={`w-4 h-4 ${summary.waitingForCeoCount > 0 ? 'text-amber-400' : 'text-zinc-500'}`} />
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-2xl bg-[#0d0d12]/95 hover:bg-[#181822] text-zinc-300 hover:text-white text-xs font-bold border border-zinc-800 backdrop-blur-xl flex items-center gap-1.5 transition-all shadow-2xl hover:border-purple-500/40"
          ...
          {/* ☀️ / 🌙 DAY - NIGHT MODE TOGGLE BUTTON */}
          <button
          ...
          >
            {timeOfDay === 'day' ? (
              <>
                ...
                <span className="hidden sm:inline">Ban Ngày ☀️</span>
              </>
            ) : (
              <>
      ...
      {activeTab === '3d_office' && (
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            {/* Navigation Helper & Interactive Clickable D-Pad */}
            ...
            {/* 📋 Creative Floating Daily Stand-Up Pill */}
            <button
              onClick={() => useOfficeStore.getState().setStandUpModalOpen(true)}
              className="pointer-events-auto px-3.5 py-2 rounded-2xl bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/40 text-purple-200 text-xs font-bold shadow-2xl backdrop-blur-xl flex items-center gap-2 transition-all hover:scale-105 active:scale-95 group cursor-pointer"
              title="Mở báo cáo Họp Đầu Ngày với A01"
            >
              <span className="w-2 h-2 rounded-full bg-purple-400 animate-ping" />
              <BookUser className="w-3.5 h-3.5 text-purple-300" />
              <span>Họp Đầu Ngày (A01)</span>
            </button>
          </div>
